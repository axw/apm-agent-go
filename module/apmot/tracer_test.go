...
func TestStartSpanFromContextMixed(t *testing.T) {
	...
	payloads := recorder.Payloads()
	require.Len(t, payloads.Transactions, 1)
	require.Len(t, payloads.Spans, 4)

	assert.Equal(t, "ot2", payloads.Spans[0].Name)
	assert.Equal(t, "apm2", payloads.Spans[1].Name)
	assert.Equal(t, "ot1", payloads.Spans[2].Name)
	assert.Equal(t, "apm1", payloads.Spans[3].Name)
